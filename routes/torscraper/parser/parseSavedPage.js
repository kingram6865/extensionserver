...
export function parseSavedPage(row) {
  if (!row?.content_html) {
  ...
  const profileData = profile.extract ? profile.extract(document, row) : {};
  ...
  const info = extractedInfo || null;
  const category = extractedCategory || null;
